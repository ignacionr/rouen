    ...
    if (ImGui::Begin("Font Character Check")) {
        ...
        if (test_char[0] != 0) {
            ...
            if ((test_char[0] & 0x80) == 0) {
                ...
                // 3-byte character
                codepoint = (static_cast<unsigned int>(test_char[0] & 0x0F) << 12) | (static_cast<unsigned int>(test_char[1] & 0x3F) << 6) | static_cast<unsigned int>(test_char[2] & 0x3F);
            } else if ((test_char[0] & 0xF8) == 0xF0 && test_char[1] != 0 && test_char[2] != 0 && test_char[3] != 0) {
                // 4-byte character (emoji range)
                codepoint = (static_cast<unsigned int>(test_char[0] & 0x07) << 18) | (static_cast<unsigned int>(test_char[1] & 0x3F) << 12) |
                            (static_cast<unsigned int>(test_char[2] & 0x3F) << 6) | static_cast<unsigned int>(test_char[3] & 0x3F);
            }
        }
