...
public:
    ...
    std::vector<CardMetricEntry> get_all_metrics() const {
        std::lock_guard<std::mutex> lock(mutex_);
        std::vector<CardMetricEntry> result;
        result.reserve(metrics_.size());
        auto now = std::chrono::steady_clock::now();
        for (const auto& [key, entry] : metrics_) {
            // Only include cards active/seen in the last 5 seconds
            if (std::chrono::duration_cast<std::chrono::seconds>(now - entry.last_updated).count() < 5) {
