...
private:
    ...
    void load_script(std::string_view locator) {
        ...
        if (loc.starts_with("js:")) {
            loc = loc.substr(3);
        }
        locator_ = loc;

        std::filesystem::path script_path{locator_};
        if (!std::filesystem::exists(script_path)) {
            // Check scripts/ user directory
            script_path = rouen::platform::get_user_data_path() / "scripts" / locator_;
        }
        ...
        if (std::filesystem::exists(script_path) && std::filesystem::is_regular_file(script_path)) {
            ...
            name(script_path.filename().string());
        } else {
            script_code_ = std::string(locator_);
            name(locator_.empty() ? "JavaScript Extension Card" : locator_);
        }
        eval_render();
    }
